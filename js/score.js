...
function buildScoreboard() {
  ...
  for (var n = 0; n < 11; n++) {
    ...
    for (var s = 0; s < score_arr.length; s++) {
      if (n == 0) {
        scoreHTML +=
          '<div class="list fontScoreList resizeFont" style="width:' +
          score_arr[s].percent +
          "%; text-align:'" +
          score_arr[s].align +
          "';\">" +
...
function toggleScorePage(con) {
  if (con) {
    ...
    scoreListsData.newPage =
      scoreListsData.newPage > totalScorePage
        ? totalScorePage
        : scoreListsData.newPage;
  } else {
    scoreListsData.newPage--;
    scoreListsData.newPage =
      scoreListsData.newPage < 1 ? 1 : scoreListsData.newPage;
  }
...
function loadScoreboard(type) {
  ...
  var pageLimit = String(
    (scoreListsData.page - 1) * 10 + "," + scoreListsData.page * 10
  );
  $.ajax({
    type: "POST",
    url: "topRank.php",
      ...
      for (var i = 0; i < scoreList.length; i++) {
        if (typeof scoreList[i] != "undefined") {
          $("ul.scoreList li")
            ...
            .find("div")
            .eq(1)
            .html(scoreList[i].name);
          $("ul.scoreList li")
            .eq(i + 1)
            .find("div")
            .eq(2)
            .html(scoreList[i].type);
          ...
          if (scoreMode == "score") {
            $("ul.scoreList li")
              ...
              .find("div")
              .eq(3)
              .html(scoreList[i].score);
          } else {
            $("ul.scoreList li")
              .eq(i + 1)
              .find("div")
              .eq(3)
              .html(millisecondsToTime(scoreList[i].score));
          }
        }
      }
    }
